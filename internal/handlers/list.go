...
func (handler *ListHandler) ToJniRepresentation(env *jnigi.Env, elementType string, value any) (*jnigi.ObjectRef, error) {
	...
	case StringClasspath:
		...
		return createGenericContainerReference(env, arr, handler.ClassHandlers)
...
func createJavaList[T any](env *jnigi.Env, size int, handlers map[string]extensions.IHandler) (*iterableRef[T], error) {
